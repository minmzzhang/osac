...
- `coderabbitai[bot]` `APPROVED` on the **exact current HEAD** (blocked while a human still has `CHANGES_REQUESTED`). Auto-start: same-repo via thin `e2e-on-approval` (`uses` test-infra `e2e-on-label`); forks via thin `e2e-on-approval-fork` (`uses` test-infra fork replay). `fork-handoff` stays a top-level job here so the replay gate can match it. `lgtm` / `/e2e-ready` still work. A later `lgtm` while those full-install runs are already active must not POST new in_progress `e2e-*-gate` checks (test-infra `e2e-start` skips invalidation when every caller would skip rerun). `/e2e-ready` `workflow_dispatch` is not on the PR SHA, so it must not POST either — unbound checks land on unrelated suites (ok-to-test cleanup).
